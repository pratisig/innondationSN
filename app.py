...
def safe_get_info(ee_obj):
    ...
    try:
        ...
        st.warning(f"⚠️ Erreur GEE : {str(e)[:50]}")
        return None
...
def create_pdf_report(df, country, p1, p2, stats):
    ...
    pdf.multi_cell(190, 5, 
        ...
        "OSMnx (OpenStreetMap), SRTM (USGS), GSW (JRC), GAUL (FAO). "
...
if not sel_a1:
    st.info("ℹ️ Veuillez sélectionner au moins une région (Admin 1).")
    st.stop()
...
def get_flood_detection(aoi_json, ref_start_str, ref_end_str, flood_start_str, flood_end_str):
    ...
    # ✅ CORRECTION: Évaluer le count IMMÉDIATEMENT avec safe_get_info()
    ...
    # ✅ Retourner s1_count évalué (int, pas objet EE)
...
# ─────────────────────────────────────────────────────────
# SÉRIE TEMPORELLE (Optionnel)
# ─────────────────────────────────────────────────────────
if analysis_mode == "Série Temporelle":
    ...
    with st.spinner("Calcul de la série temporelle..."):
        for i in range(len(dates)-1):
            ...
            f, r, count = get_flood_detection(geom_ee.getInfo(), str(ref_start), str(ref_end), d1, d2)
            
            if f and count and count > 0:
        ...
        if ts_rows:
            ...
            st.line_chart(ts_df.set_index("Date"), use_container_width=True)
        else:
            st.warning("⚠️ Pas de données pour la série temporelle.")
...
with st.spinner("Analyse GEE & OSMnx en cours..."):
    ...
    # ✅ CORRECTION: Vérifier que s1_count est un int/float, pas None
