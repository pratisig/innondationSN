...
try:
    ...
    # On capture tout (ImportError, AttributeError, etc.) pour garantir le fallback
    # Ceci évite le crash si geemap a un conflit de dépendance (ex: issue avec 'box')
    print(f"Erreur lors de l'import des librairies géospatiales : {e}")
    HAS_GEE = False
...
def initialize_gee():
    ...
    if not HAS_GEE:
        return False, "Librairies géospatiales manquantes ou erreur d'import (ee, geemap, geopandas)."
